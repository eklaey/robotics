...
#         print("Warning: Received empty or invalid frame due to Wi-Fi lag. Skipping this iteration...")
...
while r.go_on():
    ...
    # 1. Fetch exactly ONE frame per iteration (Fast & Responsive)
    frame, markers = threaded_cam.get_latest()
    
    if frame is None or frame.size == 0:
        time.sleep(0.01)
        continue

    # 2. Extract pose inline (Just like circular_path.py!)
    if markers and MARKER_ID in markers:
        tvec = np.array(markers[MARKER_ID]['tvec']).reshape(3, 1)
        t_w = R_wc @ tvec + C_w
        tx = t_w[0].item()
        ty = t_w[1].item()
    ...
    if not startup_done:
        if resync_start_time is None:
            resync_start_time = time.time()
        if time.time() - resync_start_time < RESYNC_DURATION:
            r.set_speed(0, 0)
            continue
        startup_done = True
        resync_start_time = None
    ...
    # 5. State Machine & Robot Movement
    if state == BASE:
        ...
        r.set_speed(NORM_SPEED - ds_left, NORM_SPEED - ds_right)
        #time.sleep(0.03)
    ...
    if frame_count % 50 == 0:  
        # WARNING: plt.pause() forces the entire script to sleep for a moment.
        # If your robot is still a tiny bit sluggish, comment these 4 lines out!
        plt.clf()
        plt.imshow(grid, origin='lower', cmap='gray')
        plt.title("Map")
        plt.pause(0.001)
