...
###################### ROBOT SETUP #####################################
########################################################################
# Allow IP to be passed from the command line (via run_multiple.sh)
if len(sys.argv) == 2:
    MY_IP = sys.argv[1]
else:
    MY_IP = '192.168.2.202' # Fallback IP
...
while r.go_on():
    ...
    if state == BASE:
        ...
        if mode == EXPLORER:
            ...
            if detections:
                valid_targets = [d for d in detections if d.label == ASSIGNED_TARGET]
                
                if valid_targets:
                    target = max(valid_targets, key=lambda d: d.area)
                    target_color = ASSIGNED_COLOR_ID
        ...
        # -------------------------------------------------------------
            ...
            if ((tof_distance < TARGET_DISTANCE or (prox_values[0] > 500 or prox_values[7] > 500))
                ...
                # --- TRIGGER TRANSMISSION PHASE ---
                print(f"+++ GOAL FOUND! SWITCHING TO TRANSMITTING STATE +++")
                mode = TRANSMITTING
                continue
        ...
        # -------------------------------------------------------------
        # --- MODE: TRANSMITTING (Exchange Map Data) ------------------
        # -------------------------------------------------------------
        elif mode == TRANSMITTING:
            r.set_speed(0, 0)
            
            # Assigned coordinates to share with other robot based on our target color
            if ASSIGNED_TARGET == "Red" and red_goal_grid is not None:
                my_bx, my_by = red_goal_grid
            elif ASSIGNED_TARGET == "Green" and green_goal_grid is not None:
                my_bx, my_by = green_goal_grid
            else:
